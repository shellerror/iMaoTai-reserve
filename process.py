...
def get_current_session_id():
    ...
    logging.info(f"获取Session ID URL: {my_url}")
    responses = requests.get(my_url)
    logging.info(f"获取Session ID 响应: {responses.text[:200]}")
    ...
    try:
        ...
        if 'data' in result and 'sessionId' in result['data']:
            current_session_id = result['data']['sessionId']
            dict.update(headers, {'current_session_id': str(current_session_id)})
            logging.info(f"Session ID 获取成功: {current_session_id}")
        else:
            logging.error(f"Session ID 获取失败: 响应格式异常 {result}")
            dict.update(headers, {'current_session_id': '0'})
    except Exception as e:
        logging.error(f"Session ID 解析失败: {e}")
        dict.update(headers, {'current_session_id': '0'})
...
def get_location_count(province: str,
    ...
    # URL编码省份名称
    encoded_province = quote(province, safe='')
    url = f"https://static.moutai519.com.cn/mt-backend/xhr/front/mall/shop/list/slim/v3/{session_id}/{encoded_province}/{item_code}/{day_time}"
    logging.info(f"门店列表URL: {url}")
